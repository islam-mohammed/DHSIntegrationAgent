...
public sealed class AttachmentDispatchService : IAttachmentDispatchService
{
    ...
    private async Task ProcessSingleBatchAttachmentsAsync(long batchId, IProgress<WorkerProgressReport> progress, CancellationToken ct)
    {
        try
        {
            ...
            await Parallel.ForEachAsync(claimGroups, options, async (group, cancellationToken) =>
            {
                ...
                foreach (var (att, initialRow) in group)
                {
                    ...
                    try
                    {
                        ...
                        var currentUploaded = Interlocked.Increment(ref uploadedCount);

                        // Report Progress (0-70%)
                        double percentage = (double)currentUploaded / totalAttachments * 70.0;
                        progress.Report(new WorkerProgressReport("StreamC",
                            $"Uploading {currentUploaded} of {totalAttachments}",
                            BatchId: batchId,
                            Percentage: percentage,
                            ProcessedCount: currentUploaded,
            ...
            await Parallel.ForEachAsync(claimGroups, options, async (group, cancellationToken) =>
            {
                ...
                var currentNotified = Interlocked.Increment(ref notifiedClaimsCount);

                // Report Progress (70% -> 100%)
                double percentage = 70.0 + ((double)currentNotified / totalClaimsWithAttachments * 30.0);
                progress.Report(new WorkerProgressReport("StreamC",
                    $"Sending {currentNotified} of {totalClaimsWithAttachments}",
                    BatchId: batchId,
                    Percentage: percentage,
                    ProcessedCount: currentNotified,
