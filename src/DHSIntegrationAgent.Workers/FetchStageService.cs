...
public sealed class FetchStageService : IFetchStageService
{
    ...
    public FetchStageService(
        ...
        IClaimExtractionPipeline pipeline,
    ...
    public async Task ProcessBatchAsync(BatchRow batch, IProgress<WorkerProgressReport> progress, CancellationToken ct)
    {
        ...
        // 2. Resolve payer settings and PayerCode if missing
        ...
        if (string.IsNullOrWhiteSpace(bcrId))
        {
            ...
            var totalClaimsCount = await _pipeline.CountClaimsAsync(batch.ProviderDhsCode, batch.CompanyCode, startDate, endDate, ct);
        ...
        var totalClaims = await _pipeline.CountClaimsAsync(batch.ProviderDhsCode, batch.CompanyCode, batchStartDate, batchEndDate, ct);
        ...
        await using (var uow = await _uowFactory.CreateAsync(ct))
        {
            ...
            if (providerProfile?.FetchClaimCountPerThread > 0)
                pageSize = providerProfile.FetchClaimCountPerThread;

            var counts = await uow.Claims.GetBatchCountsAsync(batch.BatchId, ct);
            processedCount = counts.Total;

            var lastSeenKey = await uow.Claims.GetMaxProIdClaimAsync(batch.BatchId, ct);
            if (lastSeenKey.HasValue)
                cursor = ResumeCursor.FromInt(lastSeenKey.Value);
        }

        if (processedCount > 0)
            _logger.LogInformation("Resuming fetch for batch {BatchId} from cursor {Cursor}, processed {ProcessedCount}/{TotalClaims}",
                batch.BatchId, cursor?.Value, processedCount, totalClaims);

        progress.Report(new WorkerProgressReport("StreamA",
            $"Fetching {processedCount} of {totalClaims} from HIS system",
            BatchId: batch.BatchId, ProcessedCount: processedCount, TotalCount: totalClaims, BcrId: bcrId));
        ...
        // Rescan staged claims for domain discovery when resuming.
        if (processedCount > 0)
        {
            ...
            await foreach (var payloadBytes in uow.ClaimPayloads.GetPayloadsByBatchAsync(batch.BatchId, ct))
            {
                ...
                try
                {
                    ...
                    if (bundle != null) DiscoverDomainValues(bundle, domainLookup, discoveredValues);
                }
                catch (Exception ex) { _logger.LogWarning(ex, "Failed to deserialize staged payload during rescan."); }
            }
        }

        while (true)
        {
            var page = await _pipeline.GetNextPageAsync(
                batch.ProviderDhsCode, batch.CompanyCode,
                batchStartDate, batchEndDate,
                pageSize, cursor, ct);

            if (page.ClaimKeys.Count == 0) break;

            var rawBundles = await _pipeline.FetchBundlesAsync(batch.ProviderDhsCode, page.ClaimKeys, ct);
            ...
            foreach (var rawBundle in rawBundles)
            {
                ...
                {
                    double pct = ((double)processedCount / totalClaims) * 30;
                    progress.Report(new WorkerProgressReport("StreamA",
                        $"Fetching {processedCount} of {totalClaims} from HIS system",
                        Percentage: pct, BatchId: batch.BatchId,
                        ProcessedCount: processedCount, TotalCount: totalClaims));
                }
            }

            cursor = page.NextCursor ?? (page.ClaimKeys.Count > 0 ? ResumeCursor.FromInt(page.ClaimKeys[^1]) : cursor);
                ...
                foreach (var item in chunk)
                {
                        ...
                        && item.PayloadBytes is not null && item.Sha256 is not null)
                    {
                        await uow.Claims.UpsertStagedAsync(
                            batch.ProviderDhsCode, item.ProIdClaim,
                            batch.CompanyCode, batch.MonthKey,
                            batch.BatchId, bcrId, _clock.UtcNow, ct);

                        await uow.ClaimPayloads.UpsertAsync(
                            new ClaimPayloadRow(
                                new ClaimKey(batch.ProviderDhsCode, item.ProIdClaim),
                                item.PayloadBytes, item.Sha256, 1,
                                _clock.UtcNow, _clock.UtcNow), ct);
                    }
                }
        ...
        progress.Report(new WorkerProgressReport("StreamA",
            $"Batch {batch.BatchId} staged successfully. {processedCount} claims processed.",
            BatchId: batch.BatchId, ProcessedCount: processedCount, TotalCount: totalClaims));
    }
    ...
    private static void ProcessSection(
        ...
        HashSet<(string DomainName, DomainTableId DomainTableId, string Value)> discovered)
    {
        if (!domainLookup.TryGetValue(sectionName, out var sectionDomains)) return;

        foreach (var kv in obj)
        {
            if (kv.Value == null) continue;
            var keyLower = kv.Key.ToLowerInvariant();
            if (sectionDomains.TryGetValue(keyLower, out var doms))
            {
                var val = kv.Value.ToString().Trim().Trim('"');
                if (string.IsNullOrWhiteSpace(val)) continue;
                foreach (var d in doms)
                    discovered.Add((d.DomainName, d.DomainTableId, val));
            }
        }
    }
    ...
    private async Task<int> UpsertMissingMappingsAsync(string providerDhsCode, IReadOnlyList<ScannedDomainValue> values, CancellationToken ct)
    {
        ...
        foreach (var mm in values)
        {
            ...
            await uow.DomainMappings.UpsertDiscoveredAsync(
                providerDhsCode, mm.DomainName, (int)mm.DomainTableId,
                mm.Value, DiscoverySource.Scanned, _clock.UtcNow, ct,
                providerNameValue: mm.Value, domainTableName: mm.DomainName);
            discovered++;
        }
    ...
    private static string RemoveUnUTF8Characters(string input)
    {
        ...
        try
        {
            return UnUtf8Regex.Replace(input, " ")
    ...
    private void SanitizeNode(JsonNode? node)
    {
        ...
        if (node is JsonObject obj)
        {
            ...
            {
                if (kv.Value is JsonValue jVal && jVal.GetValueKind() == System.Text.Json.JsonValueKind.String)
                {
                    if (jVal.TryGetValue<string>(out var s))
                    {
                        var sanitized = RemoveUnUTF8Characters(s);
                        if (sanitized != s) obj[kv.Key] = sanitized;
                    }
                }
                else SanitizeNode(kv.Value);
            }
        }
        else if (node is JsonArray arr)
        {
            ...
            {
                var item = arr[i];
                if (item is JsonValue jVal && jVal.GetValueKind() == System.Text.Json.JsonValueKind.String)
                {
                    if (jVal.TryGetValue<string>(out var s))
                    {
                        var sanitized = RemoveUnUTF8Characters(s);
                        if (sanitized != s) arr[i] = sanitized;
                    }
                }
                else SanitizeNode(item);
            }
        }
    }
    ...
    private static void RemoveOldIdentifiers(JsonNode? node)
    {
        ...
        if (node is JsonObject obj)
        {
            var keysToRemove = obj.Select(k => k.Key)
                .Where(k =>
                {
                    var clean = k.Replace("_", "").Replace("-", "").Replace(" ", "");
                    return string.Equals(clean, "providerdhscode", StringComparison.OrdinalIgnoreCase) ||
                           string.Equals(clean, "providercode", StringComparison.OrdinalIgnoreCase) ||
                           string.Equals(clean, "bcrid", StringComparison.OrdinalIgnoreCase);
                })
                .ToList();
            foreach (var k in keysToRemove) obj.Remove(k);
            foreach (var kv in obj) RemoveOldIdentifiers(kv.Value);
        }
        else if (node is JsonArray arr)
        {
            foreach (var item in arr) RemoveOldIdentifiers(item);
        }
    }
}
